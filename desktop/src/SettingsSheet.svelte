<script lang="ts">
  ...
  let tab=$state<'service'|'account'|'connections'|'tasks'|'appearance'>('service'); let health=$state<ServiceHealth|null>(null); let runtime=$state('');let error=$state('');let busy=$state(false);let account=$state<Record<string,unknown>|null>(null);
  async function load(){busy=true;error='';try{[health,runtime]=await Promise.all([loadServiceHealth(),runtimeStatus()]);account=await trainerAction<Record<string,unknown>>('account').catch(()=>null)}catch(e){error=e instanceof Error?e.message:String(e)}finally{busy=false}}
